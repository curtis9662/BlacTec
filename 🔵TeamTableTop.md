...
* **Objective:** Test detection, analysis, and response capabilities using the Microsoft security stack.
...
* Check Advanced Hunting for similar emails across the organization.
    * **Query:**
        ```kusto
        EmailEvents
        | where AttachmentCount > 0 and SenderFromAddress contains "suspicious-domain.com"
        ```
...
* Recommend policy changes: Block Office macros from the internet.
...
### Scenario
Threat actor attempts to dump LSASS memory using a Mimikatz variant to harvest credentials.
...
Find potential compromise indicators across all devices:
```kusto
DeviceProcessEvents
